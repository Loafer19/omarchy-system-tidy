...
- **Packages** — everything installed beyond Omarchy's defaults, with size
  and install date. Toggle **Orphans** for packages nothing depends on
  anymore. One-click remove either way.
...
- **Cleanup** — pacman cache, coredumps, trash, Docker, browser cache,
  journal logs. Each row says what it does and doesn't touch.
...
with it. Docker cleanup is `docker system prune -f`, no `-a`, so images
you're using stay. Nothing runs without a click.
